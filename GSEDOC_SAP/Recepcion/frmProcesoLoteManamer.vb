...
Public Class frmProcesoLoteManamer
    ...
    Public Sub CreaFormularioProcesoLoteManamer()
        Dim xmlDoc As New Xml.XmlDocument
        Dim strPath As String
        rsboApp.SetStatusBarMessage(NombreAddon + " - Consultando, Espere Por favor!!", SAPbouiCOM.BoMessageTime.bmt_Short, False)
        If RecorreFormulario(rsboApp, "frmProcesoLoteManamer") Then
            Exit Sub
        End If

        strPath = System.Windows.Forms.Application.StartupPath & "\frmProcesoLoteManamer.srf"
        xmlDoc.Load(strPath)
        ...
        Try
            ...
            Catch exx As Exception
                rsboApp.Forms.Item("frmProcesoLoteManamer").Close()
                xmlDoc = Nothing
                Exit Sub
            End Try
            oForm = rsboApp.Forms.Item("frmProcesoLoteManamer")
            ...
            oForm.DataSources.DataTables.Item("dtDocs").Columns.Add("Observación", SAPbouiCOM.BoFieldsType.ft_AlphaNumeric, 100)
    ...
    Private Sub VerificarSN()
        ...
        Try
            Dim oForm As SAPbouiCOM.Form = rsboApp.Forms.Item("frmProcesoLoteManamer")
                                ...
                                If Guarda_DocumentoRecibido_RE(DocEntryFacturaRecibida_UDO, oRetencion) Then
                                    ...
                                    Try
                                        If Exitoso = True Then
                                            ...
                                            Dim odt As SAPbouiCOM.DataTable = rsboApp.Forms.Item("frmProcesoLoteManamer").DataSources.DataTables.Item("dtDocs")
                                        ...
                                        Else
                                            Dim odt As SAPbouiCOM.DataTable = rsboApp.Forms.Item("frmProcesoLoteManamer").DataSources.DataTables.Item("dtDocs")
                                            odt.SetValue(14, _fila, "Error al crear Preliminar: " + menError.ToString)
    ...
    Private Sub rSboApp_ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean) Handles rsboApp.ItemEvent
        Try
            ...
            If pVal.FormTypeEx = "frmProcesoLoteManamer" Then
                        ...
                        If Not pVal.Before_Action Then
                            Select Case pVal.ItemUID
                                ...
                                Case "oGrid"
                                    ...
                                    oFor = rsboApp.Forms.Item("frmProcesoLoteManamer")
        ...
        Try
            ...
            Select Case cbxTipo.Value
                ...
                Case "07"
                    ...
                    Dim z = WS.ConsultarRetencion_CabeceraBuscar(oFiltrosRecepcionECRT, mensaje).ToList
                    If Not z Is Nothing Then
                        i = z.Count
...
#Enable Warning BC42104 ' La variable 'm_oProgBar' se usa antes de que se le haya asignado un valor. Podría darse una excepción de referencia NULL en tiempo de ejecución.
    ...
    Private Function ChooseFromList(ByRef pVal As SAPbouiCOM.ItemEvent, ByVal FormUID As String) As Boolean
        ...
        If FormUID = "frmProcesoLoteManamer" Then
            oForm = rsboApp.Forms.Item("frmProcesoLoteManamer")
    ...
    Private Sub SBO_Application_RightClickEvent(ByRef eventInfo As SAPbouiCOM.ContextMenuInfo, ByRef BubbleEvent As Boolean) Handles rsboApp.RightClickEvent
        ...
        Try
            If eventInfo.FormUID = "frmProcesoLoteManamer" And eventInfo.ItemUID = "oGrid" Then
...
#Enable Warning BC42030 ' La variable 'idForm' se ha pasado como referencia antes de haberle asignado un valor. Podría darse una excepción de referencia NULL en tiempo de ejecución.
                    If typeEx = "frmProcesoLoteManamer" Then
                        If ofila > 0 Then
                            Dim oForm As SAPbouiCOM.Form = rsboApp.Forms.Item("frmProcesoLoteManamer")
                            Dim oDataTable As SAPbouiCOM.DataTable = oForm.DataSources.DataTables.Item("dtDocs")
                            Dim sRUC As String = oDataTable.GetValue(4, ofila).ToString()
...
#Enable Warning BC42030 ' La variable 'idForm' se ha pasado como referencia antes de haberle asignado un valor. Podría darse una excepción de referencia NULL en tiempo de ejecución.
            ...
            End If
        Catch ex As Exception
            Utilitario.Util_Log.Escribir_Log($"Error rsboApp_MenuEvent: {ex.Message}", "frmProcesoLoteManamer")
...
#Region "Funciones Proceso Lote"
    ...
    Public Function Guarda_DocumentoRecibido_Factura(ByRef DocEntryFacturaRecibida_UDO As String, ByRef oFac As Entidades.wsEDoc_ConsultaRecepcion.ENTFactura) As Boolean
        ...
        'Dim iIdDocEdoc As Long = Long.Parse(oDataTable.GetValue(14, ofila).ToString())
        'Dim WS As New Entidades.wsEDoc_ConsultaRecepcion.WSRAD_KEY_CONSULTA
        'WS.Url = Functions.VariablesGlobales._WS_Recepcion

        '_oFactura = WS.ConsultarFactura_Detalle(Functions.VariablesGlobales._WS_RecepcionClave, iIdDocEdoc, mensaje)
        ...
        'Dim oFac As Entidades.wsEDoc_ConsultaRecepcion.ENTFactura = _oFactura
    ...
    Private Function CrearFacturaPremilinarServicio(ByRef sCardCode As String, ByVal oFactura As Entidades.wsEDoc_ConsultaRecepcion.ENTFactura, ByRef sDocEntryPreliminar As String, ByVal DocEntryFVRecibida_UDO As String) As Boolean
        ...
        Try
            ...
            If QFE = "SI" Then
                GRPO.DocDate = oFactura.FechaEmision 'fecha de contabilizacion
                'GRPO.DocDueDate = oFactura.FechaEmision 'fecha de vencimiento 
                GRPO.TaxDate = oFactura.FechaEmision  'fecha del documento
            Else

                GRPO.DocDate = Date.Now  'fecha de contabilizacion
                GRPO.TaxDate = oFactura.FechaEmision  'fecha del documento
                'GRPO.TaxDate = oFactura.FechaEmision
                'GRPO.DocDueDate = Date.Now 'fecha de vencimiento 
            End If
...
#Enable Warning BC42024 ' Variable local sin usar: 'result'.
            For Each oDetalle As Entidades.wsEDoc_ConsultaRecepcion.ENTDetalleFactura In oFactura.ENTDetalleFactura
                ...
                Utilitario.Util_Log.Escribir_Log("Obteniendo TAXCODE - QUERY: " + sQueryCodImp + "Resultado :" + CodImp.ToString(), "frmProcesoLoteManamer")
                Utilitario.Util_Log.Escribir_Log("Obteniendo TAXCODE - QUERY: " + sQueryCodImpV + "Resultado :" + CodImpV.ToString(), "frmProcesoLoteManamer")
                ...
                Utilitario.Util_Log.Escribir_Log("Obteniendo centro de costo - QUERY: " + sQueryCentroCosto + "Resultado :" + CodCentroCosto.ToString(), "frmProcesoLoteManamer")
                ...
                Utilitario.Util_Log.Escribir_Log("Obteniendo marca - QUERY: " + sQueryMarca + "Resultado :" + CodMarca.ToString(), "frmProcesoLoteManamer")
...
#Enable Warning BC42030 ' La variable 'ErrMsg' se ha pasado como referencia antes de haberle asignado un valor. Podría darse una excepción de referencia NULL en tiempo de ejecución.
    ...
    Public Function Guarda_DocumentoRecibido_RE(ByRef DocEntryFacturaRecibida_UDO As String, ByRef oRetencion As Entidades.wsEDoc_ConsultaRecepcion.ENTRetencion) As Boolean
    ...
    Private Function CrearPagoRecibido_E_OCB(ByRef sCardCode As String, ByRef oRetencion As Entidades.wsEDoc_ConsultaRecepcion.ENTRetencion, ByRef sDocEntryPreliminar As String, DocEntryRERecibida_UDO As String) As Boolean
        ...
        Try
            ...
            Querycomentario = "Select ""U_Code"",""U_Comentario"" from ""@GS_COMEN_PRCB"" Where ""U_Code"" =  '" + sCardCode + "'"
            _sn = oFuncionesB1.getRSvalue(Querycomentario, "U_Code", "")
            Utilitario.Util_Log.Escribir_Log("query SN : " + Querycomentario.ToString + " Resultado: " + _sn.ToString(), "ProcesoLote")
            _comen = oFuncionesB1.getRSvalue(Querycomentario, "U_Comentario", "")
            Utilitario.Util_Log.Escribir_Log("query SN comentario: " + Querycomentario.ToString + " Resultado: " + _comen.ToString(), "ProcesoLote")
            If sCardCode = _sn Then
                vPay.JournalRemarks = _comen
            Else
                vPay.JournalRemarks = ""
            End If
            'If IdRucEmp = "1790027791001" Then 'manamer
                                ...
                                Try
                                    If ActualizadoEstado_DocumentoRecibido_RE(idDocumentoRecibido_UDO, "docFinal", sClaveAcceso) Then
                                        ...
                                        Try ' SI ESTA OCULTO E FORMULARIO SE CAE
                                            If rsboApp.Forms.Item("frmProcesoLoteManamer").Visible = True Then
                                                rsboApp.Forms.Item("frmProcesoLoteManamer").Freeze(True)
                                                Dim odt As SAPbouiCOM.DataTable = rsboApp.Forms.Item("frmProcesoLoteManamer").DataSources.DataTables.Item("dtDocs")
                                                odt.Rows.Remove(ofila)
                                                rsboApp.Forms.Item("frmProcesoLoteManamer").Freeze(False)
                                            End If
                                        Catch ex As Exception
                                        End Try
                                    End If
