...
Public Class frmConsultasDB_RE
    ...
    Public Sub CargaFormularioParametrosConsultasBD()
        ...
        If RecorreFormulario(rsboApp, "frmConsultasDB_RE") Then
            Exit Sub
        End If

        strPath = System.Windows.Forms.Application.StartupPath & "\frmConsultasDB_RE.srf"
        xmlDoc.Load(strPath)
        Try
            Try
                rsboApp.LoadBatchActions(xmlDoc.InnerXml)
            Catch exx As Exception
                rsboApp.Forms.Item("frmConsultasDB_RE").Close()
                xmlDoc = Nothing
                Exit Sub
            End Try

            oForm = rsboApp.Forms.Item("frmConsultasDB_RE")
    ...
    Private Sub CargaDatos()
        ...
        Try
            ...
            For i = 0 To odt.Rows.Count - 1
                ...
                If odt.GetValue("U_Nombre", i).ToString().Equals("Query_ReDocumentosMarcados") Then
                    oForm.Items.Item("txtRDMark").Specific.value = Utilitario.Util_Encriptador.Desencriptar(odt.GetValue("U_Valor", i).ToString().Replace("{", "").Replace("}", "").ToString(), sKey)
                ElseIf odt.GetValue("U_Nombre", i).ToString().Equals("Query_ReDocumentosIntegrados") Then
                    oForm.Items.Item("txtRDInte").Specific.value = Utilitario.Util_Encriptador.Desencriptar(odt.GetValue("U_Valor", i).ToString().Replace("{", "").Replace("}", "").ToString(), sKey)
...
#End Region
    ...
    Private Sub rsboApp_ItemEvent(FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean) Handles rsboApp.ItemEvent
        ...
        Try
            If pVal.FormTypeEx = "frmConsultasDB_RE" Then
                            ...
                            Select Case pVal.ItemUID
                                Case "obtnGDBGT"
                                    ...
                                    Try
                                        ...
                                        olistaDetalleConfiguracion.Add(New Entidades.ConfiguracionDetalle("Query_ReDocumentosMarcados", Utilitario.Util_Encriptador.Encriptar(oForm.Items.Item("txtRDMark").Specific.Value.ToString(), sKey)))
                                        olistaDetalleConfiguracion.Add(New Entidades.ConfiguracionDetalle("Query_ReDocumentosIntegrados", Utilitario.Util_Encriptador.Encriptar(oForm.Items.Item("txtRDInte").Specific.Value.ToString(), sKey)))
            ...
            End If
        Catch ex As Exception
            Utilitario.Util_Log.Escribir_Log("ex rSboApp_ItemEvent: " + ex.Message.ToString(), "frmConsultasDB_REGT")
