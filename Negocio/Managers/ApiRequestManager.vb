...
Public Class ApiRequestManager
    ...
    Public Function EnviarGRSolsap(factura As Entidades.RequestGuiaRemision) As Entidades.ResponseDocuments
        Try
            _auth.ActivarTLS()
            Dim token As String = _auth.ObtenerTokenAutenticacion()
            If String.IsNullOrEmpty(token) Then Return Nothing
        ...
        Catch webEx As WebException
            ...
            If respErr IsNot Nothing Then
                ...
                End Using
            End If
            If _tipoManejo = "A" Then _sboApp.SetStatusBarMessage("Error enviando GR: " & mensajeError, SAPbouiCOM.BoMessageTime.bmt_Short, True)
            Return Nothing

        Catch ex As Exception
            If _tipoManejo = "A" Then _sboApp.SetStatusBarMessage("Error enviando GR: " & ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, True)
            Return Nothing
        End Try
    End Function
